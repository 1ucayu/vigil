...
_QUOTIENT_ACTION_RID_FIELDS: tuple[str, ...] = (
    "resource_id",
    "target_resource_id",
)
# Strict whitelist of selector fields kept in the quotient action key.
# Capture-local fields (bounds, depth, element_id, parent_id,
# ancestor_chain, target handle, raw text) are deliberately excluded so
# they cannot force spurious refinement splits. ``canonical_action_key``
# is untouched — it keeps full selector fidelity for replay/dedup.
_QUOTIENT_SELECTOR_WHITELIST: tuple[str, ...] = (
    "resource_id",
    "class_name",
    "content_description",
    "nearby_text",
)
_QUOTIENT_ACTION_KEY_FIELDS: tuple[str, ...] = (
    "type",
    "resource_id",
    "target_resource_id",
    "target_text",
...
# Closed vocabulary for ``_coarse_value_class`` — see docstring.
_VALUE_CLASS_EMPTY = "empty"
_VALUE_CLASS_NUMERIC = "numeric"
_VALUE_CLASS_MONEY = "money"
_VALUE_CLASS_DATE = "date"
_VALUE_CLASS_TIME = "time"
_VALUE_CLASS_EMAIL = "email"
_VALUE_CLASS_PHONE = "phone"
_VALUE_CLASS_NONEMPTY_TEXT = "nonempty_text"
_VALUE_CLASS_VOLATILE = "*"

_VALUE_NUMERIC_RE = re.compile(r"^-?\d+([.,]\d+)?$")
_VALUE_MONEY_RE = re.compile(r"^[$€¥£]\s*-?\d+([.,]\d+)?$")
_VALUE_DATE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$|^\d{1,2}[/-]\d{1,2}[/-]\d{2,4}$")
_VALUE_TIME_RE = re.compile(r"^\d{1,2}:\d{2}(:\d{2})?(\s*[AaPp][Mm])?$")
_VALUE_EMAIL_RE = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
_VALUE_PHONE_RE = re.compile(r"^\+?[\d\s\-().]{7,}$")


def _coarse_value_class(value: Any) -> str:
    """Coarse class for a user-entered text/value payload.

    Returns one of ``{empty, numeric, money, date, time, email, phone,
    nonempty_text, "*"}``. Used only by :func:`quotient_action_key` so
    that distinct user-entered literals (``"hello"`` vs ``"goodbye"``,
    ``"100.00"`` vs ``"42"``) collapse to the same quotient action class
    without losing the empty-vs-nonempty distinction.

    The mapping is locale-agnostic and uses only generic regex patterns
    — no app-specific tokens. Volatile readouts (composite strings that
    embed times/money/dates per :func:`_is_volatile_text`) map to
    ``"*"``.
    """
    if not isinstance(value, str):
        if value is None:
            return _VALUE_CLASS_EMPTY
        return _VALUE_CLASS_NONEMPTY_TEXT
    stripped = value.strip()
    if not stripped:
        return _VALUE_CLASS_EMPTY
    if _VALUE_MONEY_RE.match(stripped):
        return _VALUE_CLASS_MONEY
    if _VALUE_NUMERIC_RE.match(stripped):
        return _VALUE_CLASS_NUMERIC
    if _VALUE_DATE_RE.match(stripped):
        return _VALUE_CLASS_DATE
    if _VALUE_TIME_RE.match(stripped):
        return _VALUE_CLASS_TIME
    if _VALUE_EMAIL_RE.match(stripped):
        return _VALUE_CLASS_EMAIL
    if _VALUE_PHONE_RE.match(stripped) and any(ch.isdigit() for ch in stripped):
        return _VALUE_CLASS_PHONE
    if _is_volatile_text(stripped):
        return _VALUE_CLASS_VOLATILE
    return _VALUE_CLASS_NONEMPTY_TEXT
...
def _quotient_selector(selector: Any) -> tuple[tuple[str, Any], ...]:
    """Canonicalize a selector to a strict whitelist of verifier-relevant
    fields, suitable for the quotient action key.

    Kept (and normalized):

    * ``resource_id`` -> :func:`_rid_canonical_root` (fallback
      :func:`_short_rid_tail`).
    * ``class_name``  -> :func:`_class_leaf`.
    * ``content_description`` -> verbatim only when non-empty,
      non-volatile, **and** not instance-correlated with the rid (i.e.,
      the rid did not require canonicalization). Otherwise dropped.
    * ``nearby_text`` -> same gating as ``content_description``.

    Dropped: ``bounds``, ``text``, ``depth``, ``element_id``,
    ``parent_id``, ``ancestor_chain``, ``target``, and every other
    capture-local handle. The full selector is still retained by
    :func:`vigil.models.fsm.canonical_action_key` for replay / dedup /
    provenance — only this quotient-side reduction is whitelisted.
    """
    if isinstance(selector, dict):
        items: list[tuple[str, Any]] = [(str(k), v) for k, v in selector.items()]
    ...
    for field in _QUOTIENT_SELECTOR_WHITELIST:
        ...
        if field == "resource_id":
            value = new_rid if isinstance(value, str) and value else _hashable(value)
        elif field == "class_name":
            value = _class_leaf(value) if isinstance(value, str) and value else _hashable(value)
        elif field in ("content_description", "nearby_text"):
            if not isinstance(value, str) or not value.strip():
                continue
            if _is_volatile_text(value):
                continue
            if rid_reduced:
                # Likely instance-correlated with the rid; drop.
                continue
            value = value.strip()
...
def quotient_action_key(action: Any) -> tuple[tuple[str, Any], ...]:
    """Per-instance-stripped action signature for quotient refinement.

    Differs from ``vigil.models.fsm.canonical_action_key`` in four ways:

    1. Selector-bearing rid fields (``resource_id``,
       ``target_resource_id``, and the ``resource_id`` key of
       ``target_selector``) are reduced via :func:`_rid_canonical_root`
       so per-instance segments collapse into one quotient action class.
    2. ``target_selector`` is passed through :func:`_quotient_selector`,
       which strict-whitelists verifier-relevant selector fields and
       drops every capture-local handle.
    3. User-entered ``text`` / ``value`` payloads are normalized via
       :func:`_coarse_value_class` — so distinct literals like
       ``"hello"`` vs ``"goodbye"`` both collapse to ``"nonempty_text"``.
       Empty vs non-empty is still preserved.
    4. ``target_text`` is dropped when the rid was reduced (instance
       correlation), and otherwise also normalized via
       :func:`_coarse_value_class` so per-row label literals never enter
       the quotient key.

    The raw :func:`canonical_action_key` is **not** replaced — callers
    keep using it for replay / dedup / provenance.
    """
    if not isinstance(action, dict):
        return tuple()
    rid_reduced = False
    out: dict[str, Any] = {}
    for field in _QUOTIENT_ACTION_KEY_FIELDS:
        value = action.get(field)
        if field in _QUOTIENT_ACTION_RID_FIELDS and isinstance(value, str) and value:
            root = _rid_canonical_root(value)
            new_value = root or _short_rid_tail(value)
            if new_value != value:
                rid_reduced = True
            value = new_value
        elif field == "target_selector":
            value = _quotient_selector(value)
        elif field in ("text", "value"):
            value = _coarse_value_class(value) if value is not None else None
        else:
    ...
    if rid_reduced:
        ...
        tgt_text = out.get("target_text")
        if isinstance(tgt_text, str) and tgt_text:
            out["target_text"] = _coarse_value_class(tgt_text)
