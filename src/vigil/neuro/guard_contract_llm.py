...
from vigil.neuro.exploration_inputs import EXPLORATION_SYNTHETIC_INPUT_VALUES
...
def build_guard_user_prompt(
    ...
    *,
    ...
    synthetic_values = ", ".join(repr(value) for value in EXPLORATION_SYNTHETIC_INPUT_VALUES)
    sections.append(
        "[Exploration Synthetic Inputs]\n"
        "Purpose: Values generated by the exploration harness to exercise text fields. "
        "They are evidence that a field accepts input, not user intent values.\n"
        f"- synthetic_input_values: [{synthetic_values}]\n"
        "- Never copy these values into a literal guard. For input_binding, bind the "
        "typed value to a declared $intent.* slot instead."
    )
